...
DATA_FILE = os.path.join(os.path.dirname(__file__), "data", "temiz_biyoloji.txt")
BATCH_SIZE = 64
...
def sample_names(n: int = 10, max_new_tokens: int = 20):
    model.eval()
    start = torch.full((n, 1), newline_id, dtype=torch.long, device=device)
    out = model.generate(start, max_new_tokens=max_new_tokens, temperature=1.0,
                         top_k=None, eos_id=newline_id)
    model.train()
    names = []
...
torch.save({"model": model.state_dict(), "cfg": cfg}, "tiny_biyoloji.pt")

print("\nsaved checkpoint to tiny_biyoloji.pt")