...
# Hyperparameters
# ---------------------------------------------------------------------------
DATA_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "movie_titles.txt"))
TOKENIZER_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "bpe_tokenizer.json"))
BATCH_SIZE = 64
BLOCK_SIZE = 16
STEPS = 1000  # Reduced step count for faster toy training demo
LEARNING_RATE = 3e-3
EVAL_EVERY = 200
SEED = 1337
...
if not os.path.exists(TOKENIZER_FILE):
    raise FileNotFoundError(f"Tokenizer not found at {TOKENIZER_FILE}. Please run train_tokenizer.py first.")
...
# Encode the entire corpus of titles
...
def sample_names(n: int = 10, max_new_tokens: int = 20):
    model.eval()
    start = torch.full((n, 1), tokenizer.bos_token_id, dtype=torch.long, device=device)
    out = model.generate(start, max_new_tokens=max_new_tokens, temperature=1.0,
                         top_k=None, eos_id=tokenizer.eos_token_id)
    model.train()
    return [tokenizer.decode(row[1:]) for row in out.tolist()]


# ---------------------------------------------------------------------------
# Training loop
# ---------------------------------------------------------------------------
checkpoint_path = os.path.join(os.path.dirname(__file__), "tiny_qwen35.pt")
